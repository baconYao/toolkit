...
const randomStringSource = "abcdefgijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789_+"

// Tools is the type used to instantiate this module. Any variable of this type will have
// access to all the methods with the reciever *Tools
type Tools struct {
	MaxFileSize        int
...
// UploadedFile is a struct used to save information about an upload file
...
func (t *Tools) Slugify(s string) (string, error) {
	if s == "" {
		return "", errors.New("empty string no t permitted")
	}
...
func (t *Tools) ReadJSON(w http.ResponseWriter, r *http.Request, data interface{}) error {
	...
	if err != nil {
		...
		case errors.As(err, &unmarshalTypeError):
			if unmarshalTypeError.Field != "" {
				return fmt.Errorf("body contains incorrect JSON type of field %q", unmarshalTypeError.Field)
			}
	...
	// 用來檢查 body 是否含有多個 json value。我們期望只有一個。
...
// WriteJSON takes a reponse status code and arbitrary data and writes json to client
